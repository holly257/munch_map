...
const mapBoxKey =
    'pk.eyJ1IjoiaG9sbHktMjkzODQ3IiwiYSI6ImNrNTlycHgyZjBlc20zb24zZHhvbGpnaGgifQ.4wuSuhP7Za_lKtKMiGx2lg';
...
function displayResults(responseJson) {
    ...
    for (let i = 0; i < responseJson.restaurants.length; i++) {
        ...
        console.log(restauPath);
        $('#results-list').append(
            `<section class='each-result'>
                <image class='result-image' src='${restauPath.featured_image}' alt='No Image'></image>
                <div class='result-text'>
                    <h4 class='result-name'>${restauPath.name}</h4>
                    <span class="fa fa-star checked"></span>
                    <span class='result-rating'>${restauPath.user_rating.rating_text} (${restauPath.user_rating.aggregate_rating})</span>
                    ...
                    <p>
                    <h6 class='result-hours'>Hours: ${restauPath.timings}</h6>
                    <h6 class='result-cuisine'>Cuisine(s): ${restauPath.cuisines}</h6>
                    <h6 class='result-address'>Address: ${restauPath.location.address}</h6>
                <div>
            </section>`
        );
...
function getRestaurantList(cityId, categoryGiven) {
    const newUrl =
        zomatoUrl +
        'search?' +
        `entity_id=${cityId}&entity_type=city&cuisines=${categoryGiven}&start=5&count=15`;
        ...
        .then(cities => {
            ...
            $('#cities').html(
                cities.location_suggestions
                    .map(city => `<option class="city" data-id="${city.id}" value="${city.name}"/>`)
                    .join('')
            );
        })
        .catch(err => {
